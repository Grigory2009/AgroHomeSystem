...
| 3D-модель (STEP / M3D) | Назначение детали | Особенности конструкции |
|:---|:---|:---|
...
* **9 интерактивных экранов:**
  1. `SCR_HOME` — Главный дашборд с анимированным потоком помпы, плитками параметров и индикатором общего здоровья;
  2. `SCR_DETAIL` — Интерактивные графики истории параметров (24 точки) с переключением периодов (1ч / 6ч / 24ч);
  3. `SCR_SPROUT` — Виртуальный питомец-росток («Тамагочи»), отражающий настроение и стадию вегетации;
  4. `SCR_LAMP` — Управление спектром и ШИМ-яркостью фитоосвещения;
  5. `SCR_ADVISOR` — Встроенный агро-советник с рекомендациями по коррекции климата и питания;
  6. `SCR_GAME` — Аркадная мини-игра для интерактивного взаимодействия;
  7. `SCR_SETTINGS` — Калибровка тачскрина, выбор тем, режим «Demo Waves» (генератор синусоидальных колебаний датчиков);
  8. `SCR_WIFI_SCAN` — Поиск и сортировка доступных Wi-Fi сетей по уровню RSSI;
  9. `SCR_KBD` — Полноэкранная сенсорная QWERTY-клавиатура с переключением регистров для ввода паролей.
* **Встроенный Web-сервер:** REST API и веб-дашборд на порту 80 при подключении к локальной сети.
...
├── 22 мм мин вата.stp              # STEP-модель субстратной пробки 22 мм
├── 22мм .stp                       # STEP-модель посадочного гнезда ячейки
├── Корпус нижняя часть.stp         # STEP-модель базового лотка-резервуара 260x200 мм
├── Крышка - Деталь.stp             # STEP-модель активного климатического купола
├── держатель.stp                   # STEP-модель кронштейна сервопривода
├── угол.stp                        # STEP-модель углового фиксатора
├── Штырь 10мм - Деталь.stp         # STEP-модель поворотной оси петли (10 мм)
├── Штырь 9мм - Деталь.stp          # STEP-модель поворотной оси петли (9 мм)
│
├── firmware/                       # Прошивка ESP32-S3 (PlatformIO)
│   ├── platformio.ini              # Конфигурация платы, компилятора и библиотек

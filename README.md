...
Устройство выполнено в ультракомпактном форм-факторе **260×200 мм** (ревизия корпуса **v2**) и оснащено лотком на **24 изолированные посадочные ячейки** (сетка 4×6). В каждой ячейке поддерживается контролируемая среда для корневой зоны и листового аппарата.
...
│  вместо шумных кулеров  │ затопление 24 ячеек     │ ExG биофизика      │
├─────────────────────────┼─────────────────────────┼────────────────────┤
...
Корневая система каждого растения размещается в индивидуальных пробках из минеральной ваты 22 мм в 24 посадочных гнездах. Встроенная микропомпа подает питательный раствор в верхний лоток, равномерно затапливая все 24 ячейки на 15 минут, после чего раствор самотеком сливается обратно в нижний резервуар, естественным образом насыщая корни кислородом (аэрация без шумного компрессора).

### 3. Двухстадийный Edge AI пайплайн диагностики и поячеечный трекинг (24 Cells)
Широкоугольная камера на внутренней стороне крышки делает снимки всей посадочной площади. Нейросетевой модуль выполняет:
1. **Поячеечный трекинг 24 ячеек (`GridCellTracker` в `demo/cell_tracker.py`):** независимый мониторинг всхожести семян, покрытия полога (%), хлороза, некроза и очагов плесени для каждой из 24 ячеек матрицы 4×6 с выводом интерактивной тепловой карты в веб-дашборде.
...
4. **Беспроводная прошивка (OTA Engine):** обновление прошивки ESP32-S3 без USB-кабеля через браузер на `/update` или по воздуху через PlatformIO (ArduinoOTA).
...
| **`Ячейки.stp`** | Съемный посадочный лоток на 24 ячейки (сетка 4×6) | 24 посадочных гнезда под стандартные минераловатные пробки 22 мм (Grodan / кокос). |
...
| **`22мм .stp`** / **`22 мм мин вата.stp`** | Посадочные картриджи ячеек | Калибровочные вставки под субстратные кубики 22 мм. |
...
```
                 Крышка с камерой и серво-приводом
                   ...
                   ╱                         ╱│      на угол 0°..65°
                  ┌─────────────────────────┐ │
                  │ [A1][A2][A3][A4]        │ │  ◄── 24 ячейки (22 мм)
                  │ [B1][B2][B3][B4]        │ │      Сетка 4 x 6 (v2)
                  │ [C1][C2][C3][C4]        │ │
                  │ [D1][D2][D3][D4]        │ │
                  │ [E1][E2][E3][E4]        │ │
                  │ [F1][F2][F3][F4]        │ │
